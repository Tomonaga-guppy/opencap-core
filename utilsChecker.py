...
def triangulateMultiview(CameraParamList, points2dUndistorted,
    ...
    # CameraParamList からカメラ情報を読み込み、Camera オブジェクトのリスト cameraList を作成
    # useRotationEuler が True の場合は、オイラー角を cv2.Rodrigues 関数で回転行列に変換
    cameraList = []
    ...
    # triangulate
    # points2dUndistorted を、nview_linear_triangulations 関数が受け入れられる形式に変換
    stackedPoints = np.stack(points2dUndistorted)
    pointsInput = []
    for i in range(stackedPoints.shape[1]):
        pointsInput.append(stackedPoints[:,i,0,:].T)

    # nview_linear_triangulations 関数を呼び出して、三角測量を実行
    ...
    # 外れ値除去（オプション）
    # selectCamerasMinReprojError がtrueの場合、可能な全てのカメラの組み合わせに対して三角測量を行い、再投影誤差が最小になる点の集合を選択
    ...
    # 欠損マーカーの処理 (オプション):
    # ignoreMissingMarkers が True の場合、一部のカメラで検出されなかったマーカーについて、それらを検出できたカメラのみを用いて再度三角測量を行います。
    # getMissingMarkersCameras 関数 (別途定義が必要) を用いて、欠損しているマーカーとカメラを特定します。
    # 欠損マーカーごとに、それを検出できたカメラのリスト (idx_viewedCams) を作成します。
    # idx_viewedCams に含まれるカメラのみを用いて nview_linear_triangulations 関数を呼び出し、3D点を再計算します。
    # 再計算された3D点で、元の points3d を上書きします。
